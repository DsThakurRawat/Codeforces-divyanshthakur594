...
void solve() {
     ...
     no. 1->(n+1)[left->right]
     we have to fill these n+1 with array  of [a1,a2,a3___________an+1]

     chaneka as player must assign an+1 ->btw 0 and m inclusive 
     then for each i from n to 1-> the value of ai will be equal to reminder of ai+1 mod i
     in other words ai = ai+1 mod i
     chanek wants it such that after every slot is assigned with an integer there are exactly->k distinct value
     in entire screen among all n+1 slots 
