...
void solve() {
  //  int n;
   ...
   int n,m;
    cin >> n >> m;
    // here n is number of parti , and m is numbe rof teams
    /// u are asked to write a program min and max nuber of become friends 
    // we need to form maxi number of friends
    /*
    ncr -> n / n-r * r 
    here we can achieve our goal in following ways
    1. way one maximize total number of freindship
       this can be done by put maimum number of friends in one group and 
       keep 1 frind in other groups
    2. to minimum 
        evenly distribute the member

        for n = 6, m= 3
        divide evenly 2 2 2 -> min = 3
        for maxi,
        1 1 4 -> so in this way max = 
        1 1 1 1 -> each friends
          0 0 0 -> dosti done by friend 
            0 0 
              0 

      for n = 8 m = 5
    for maxi:
       n - (m-1) -> 8 - 4 = 4-> keep them 4 friends in one grp
        total pair 6
    for mini
    3 3  2  
     mini = 4
...
for m = 1 
ebey person will be in same team

*/
   long long max_pairs = (n - m + 1) * (n - m) / 2;
   long long base_size = n / m;
   long long extra_teams = n % m;
   long long min_pairs = (m - extra_teams) * (base_size * (base_size - 1)) / 2
                        + extra_teams * (base_size + 1) * base_size / 2;
